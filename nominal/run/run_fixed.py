...
from ..dynamics.integrators.foh_fixed import FirstOrderHold as FOHFixed
from ..init.warmstart import warmstart_fixed
...
from ..scvx.fixed_final_time import SCProblem
...
def run(cfg_path: str) -> Tuple[Path, Dict[str, np.ndarray]]:  # noqa: C901, PLR0915, PLR0912
    ...
    # --- warm start ---
    X, U, sigma = warmstart_fixed(model, K)  # sigma is used by FOH initializer

    # --- SCVX objects ---
    integrator = FOHFixed(model, K, sigma)
    ...
    all_sigma = np.ones(K) * sigma  # fixed sigma, for consistency with free-time output
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", "-c", required=True, help="Path to YAML config (e.g., configs/mars_2d_fixed.yaml)")
